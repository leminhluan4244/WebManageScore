...
class ClassMod {
	...
	public function getListStudent($class) {
		...
		$sql = "SELECT * FROM Account a inner join account_has_class ac 
                on a.idAccount = ac.Account_idAccount  
                WHERE Class_idClass = '" . $class->getIdClass() . "' and permission_position = 'Sinh viên'";
		$this->connSql->Connect();
		$result = $this->connSql->conn->query($sql);
		$list = array();
		if ($result->num_rows > 0) {
